...
class WhiteboxOracleCounterexampleGenerator:
	def __init__(self, network, partitioning, starting_examples):
		self.time_limit = None
		self.whiteboxoracle = network
	...
	def _get_counterexample_from(self, words):
		...
		for w in words:
			prop_class = self.proposed_dfa.classify_word(w)
			if prop_class is not None and self.whiteboxoracle.classify_word(w) != prop_class:
				return w
		return None
	...
	def _process_new_state_except_children(self, new_cluster, new_info):
		...
		if not (new_info.output_label == self.proposed_dfa.X[new_info.dfa_state]):
			counterexample = self._counterexample_from_classification_conflict(new_info)
		elif new_info.dfa_state != full_info.dfa_state:
	...
	def _add_children_states(self, cluster):
		...
		if not state_info.explored:
			...
			for char in self.proposed_dfa.alphabet:
				next_RState, output_label = self.whiteboxoracle.get_next_RState(RState, char)
				...
				self.new_RStates.append(UnrollingInfo(next_dfa_state, path, next_RState, output_label))
	...
	def _initialise_unrolling(self):
		...
		initial_RState, output_label = self.whiteboxoracle.get_first_RState()
		self.new_RStates = [UnrollingInfo(self.proposed_dfa.q0, "", initial_RState, output_label)]

	def _cex_from_starting_dict(self, dfa):
		for cex in self.starting_dict:
			if cex != "" and dfa.classify_word(cex) != self.starting_dict[cex]:
	...
	def counterexample(self, dfa):
		...
		if not None is counterexample:
			return counterexample, counterexample_message(counterexample, self.whiteboxoracle)
...
def counterexample_message(counterexample, rnn):
	return ("returning counterexample of length " + str(len(counterexample)) + ":\t\t" + counterexample +
			", this counterexample is labeled " + str(rnn.classify_word(counterexample)) +
			" by the given RNN.")
...
class UnrollingInfo:
	def __init__(self, next_dfa_state, path, RState, output_label):
		self.explored = False
